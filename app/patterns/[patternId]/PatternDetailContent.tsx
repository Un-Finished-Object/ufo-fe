...
} from "@/lib/queries/patternPurchase";
import { userQueryKeys } from "@/lib/queries/user";
import { myChatRoomsQueryKey } from "@/src/hooks/queries/useMyChatRoomsQuery";
...
type PatternDetailContentProps = {
...
export default function PatternDetailContent({
  ...
  const purchaseStatusQuery = useQuery({
    ...patternPurchaseStatusQueryOptions(pattern.id),
    enabled: authStatus !== "loading" && isAuthenticated,
  });
  const hasChatPurchase = purchaseStatusQuery.data?.chat === true;
  const isResolvingChatPurchase =
    authStatus === "loading" || (isAuthenticated && purchaseStatusQuery.isPending);
  const shouldShowChatCreditBadge = authStatus !== "loading" && !hasChatPurchase;
  const currentCreditText = walletQuery.isPending
    ? "불러오는 중..."
    : `${walletQuery.data ?? 0} 크레딧`;

  const purchaseChatMutation = useMutation({
    mutationFn: () => purchasePatternAccess({ patternId: pattern.id, type: 1 }),
    onSuccess: (data) => {
      ...
      <main className="mx-auto min-h-screen w-full max-w-[430px] bg-ufo-surface pb-28 text-ufo-text">
        ...
        <section className="px-4">
          {pattern.errorMessage ? (
            <p className="mb-3 rounded-xl bg-ufo-brand-soft px-3 py-2 text-xs font-medium text-ufo-text">
              {pattern.errorMessage}
            </p>
          ) : null}
